               ...
               $( "#search" ).autocomplete(
                {
                  ...
                  select: function( event, ui ) {
                    // gets all the classes of a specific course
...
function createCourseTable(classes){
  ...
  for (var i = 0; i < classes.length; i++){
    ...
    var newCells = '<tbody class="section" onmouseover="showRoom('+classes[i]["SALA"]+')" onmouseout="showAllRooms()"><tr><td colspan="5" id="courseClass">Turma '+classes[i]["TURMA"]+'</td></tr>';
    ...
    if (classes[i]["HORARIO2"]==null){
      ...
      newCells += '<td colspan="5" id="courseTime" class="c1">'+classes[i]["HORARIO1"]+'</td></tr></tbody>';
    }
...
function showAllRooms(){
  ...
  for (var j = 0; j < salas.length; j++){  
    d3.select(svg).select("rect#s"+salas[j]).style("opacity","1")
  }
...
function showRoom(room){
  ...
  console.log("OBJECT!!")
    ...
    var f = room.item.value.substring(0,1);

    if (f>=1 && f <=4){
      showFloor(f);
    }
  }
  else if (typeof room === "number"){ // if is a plain number
  console.log("NUMBER!!");
   d3.select(svg).select("rect#s"+room).style("opacity","1");   

    var f = room.toString().substring(0,1);
